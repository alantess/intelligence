...
class Env(object):
    def __init__(self,csv="/media/alan/seagate/Downloads/Binance_LTCUSDT_minute_ds.csv",n_actions=3stop_loss=0.8,time_inc = 1):
        self.csv = csv
        self.cols = 0
    ...
    def _load(self):
        ...
        df = df.iloc[::-1] 
        df = df.reset_index()
        df = df.drop(['date','index','unix','symbol', 'Volume USDT' , 'tradecount::'], axis=1)
    ...
    def step(self,a):
        ...
        info = {'btc':self.crypto_wallet , 'usdt':
                self.usdt_wallet}
    ...
    def _update(self):
        self.crypto_wallet *= float(self.unscaled_df.values[self.cur_step + self.time_inc][3] / self.price)
